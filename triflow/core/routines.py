...
class F_Routine(ModelRoutine):
    def __call__(self, fields, pars):
        ...
        Args:
            fields (triflow.Fields): triflow fields container generated by
                a triflow.Model containing the actual state of the dependent
                variables and helper functions.
            pars (dict): dictionnary with the different physical parameters
                of the model and the 'periodic' key.

        Returns:
            numpy.ndarray:
                flat array containing the right hand side
    ...
    def diff_approx(self, fields, pars, eps=1E-8):
        nvar, N = len(fields._keys), fields.size
    ...
    def __call__(self, fields, pars, sparse=True):
        ...
        Returns:
            scipy.sparse.CSC or numpy.ndarray:
                sparse or dense form (depending of the `sparse` argument)
                of the Jacobian approximation of the dynamical system
                right hand side.
        """
        uargs = [fields['x'], *[fields[key] for key in self.args]]
